...
module.exports = {
  ...
  execute: async (client, message, args, data, db) => {
    ...
    if (!owners.includes(message.author.id) && message.author.id !== "693661618021859329") return

    let user = client.users.find(user => args.length && message.mentions.users.size < 1 && user.username.toLowerCase().startsWith(args.slice(0, user.username.split(" ").length).join(" ").toLowerCase())) || client.users.cache.get(args[0]) || message.mentions.users.first()
